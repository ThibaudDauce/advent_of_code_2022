...
fn compute(input: &'static str) -> (usize, usize, usize) {
    ...
    let a = compute_min_time(((1, 0, 0), 0), (max_x - 1, max_y), &blocks_by_minutes);
    let b = compute_min_time(((max_x - 1, max_y, a), a), (1, 0), &blocks_by_minutes);
    let c = compute_min_time(((1, 0, b), b), (max_x - 1, max_y), &blocks_by_minutes);

    (a, b - a, c - b)
}

fn compute_min_time(
    start_position: ((i64, i64, usize), usize),
    end_position: (i64, i64),
    blocks_by_minutes: &Vec<HashSet<(i64, i64)>>,
) -> usize {
    let mut distances: HashMap<(i64, i64, usize), usize> = HashMap::from([start_position]);
    let mut visited: HashSet<(i64, i64, usize)> = HashSet::new();
    let mut distances_to_visit = BinaryHeap::from([Reverse((start_position.1, start_position.0))]);
    loop {
    ...
    *distances
        .iter()
        .filter(|((x, y, _), _)| *x == end_position.0 && *y == end_position.1)
