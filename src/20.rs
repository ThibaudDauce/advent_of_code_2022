...
fn part1(input: &'static str) -> i64 {
    let mut digits: Vec<(usize, i64)> = input
        ...
        .lines()
        .enumerate()
        .map(|(index, line)| (index, line.trim().parse().unwrap()))
        .collect();
...
fn move_value(digits: &mut Vec<(usize, i64)>, original_index: usize) {
    let index = digits
        .iter()
    ...
    let new_index = compute_new_index(index, value.1, digits.len());
    ...
    if new_index > index {
        (&mut digits[..]).copy_within(index + 1..=new_index, index);
    } else {
        (&mut digits[..]).copy_within(new_index..index, new_index + 1);
    }

    digits[new_index] = value;
}
...
#[test]
fn test() {
    assert_eq!(1, compute_new_index(0, 1, 7));
    assert_eq!(6, compute_new_index(2, -2, 7));
    assert_eq!(3, compute_new_index(5, 4, 7));
